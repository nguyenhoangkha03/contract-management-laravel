...
<div class="fi-ta-table overflow-x-auto">
    <table class="fi-ta-table-content w-full table-auto divide-y divide-gray-200 dark:divide-white/10">
        ...
        <tbody class="fi-ta-body divide-y divide-gray-200 dark:divide-white/10">
            @forelse($contractInvoices as $index => $invoice)
                <tr class="fi-ta-row [@media(hover:hover)]:transition [@media(hover:hover)]:duration-75 hover:bg-gray-50 dark:hover:bg-white/5">
                    ...
                    <td class="fi-ta-cell p-0 first-of-type:ps-1 last-of-type:pe-1 sm:first-of-type:ps-3 sm:last-of-type:pe-3">
                        <div class="fi-ta-col-wrp px-3 py-4">
                            <div class="fi-ta-text text-sm leading-6 text-gray-950 dark:text-white">
                                {{ $invoice->issue_date ? \Carbon\Carbon::parse($invoice->issue_date)->format('d/m/Y') : 'N/A' }}
                            </div>
                        </div>
                    </td>
                    <td class="fi-ta-cell p-0 first-of-type:ps-1 last-of-type:pe-1 sm:first-of-type:ps-3 sm:last-of-type:pe-3">
                    ...
                    <td class="fi-ta-cell p-0 first-of-type:ps-1 last-of-type:pe-1 sm:first-of-type:ps-3 sm:last-of-type:pe-3">
                        <div class="fi-ta-col-wrp px-3 py-4">
                            <div class="fi-ta-text text-sm leading-6 text-gray-950 dark:text-white font-medium">
                                {{ number_format($invoice->amount ?? 0) }} VNĐ
                            </div>
                        </div>
                    </td>
                    <td class="fi-ta-cell p-0 first-of-type:ps-1 last-of-type:pe-1 sm:first-of-type:ps-3 sm:last-of-type:pe-3">
                    ...
                    <td class="fi-ta-cell p-0 first-of-type:ps-1 last-of-type:pe-1 sm:first-of-type:ps-3 sm:last-of-type:pe-3">
                        <div class="fi-ta-col-wrp px-3 py-4">
                            <div class="fi-ta-text text-sm leading-6 text-gray-950 dark:text-white max-w-xs truncate" title="{{ $invoice->note }}">
                        ...
                        <div class="fi-ta-col-wrp px-3 py-4">
                            <div class="fi-ta-actions flex shrink-0 items-center gap-3">
                                @if($invoice->file_path && \Illuminate\Support\Facades\Storage::disk('public')->exists($invoice->file_path))
                                    <a href="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($invoice->file_path) }}" 
                                       target="_blank"
                                ...
                                <button type="button"
                                        onclick="if(confirm('Bạn có chắc chắn muốn xóa hóa đơn này?')) { $wire.call('deleteContractInvoiceConfirmed', {{ $invoice->id }}) }"
                        ...
                        <div class="fi-ta-empty-state px-6 py-12">
                            <div class="fi-ta-empty-state-content mx-auto grid max-w-lg justify-items-center text-center">
                                ...
                                <p class="fi-ta-empty-state-description text-sm text-gray-500 dark:text-gray-400">
                                    Nhấn "Thêm" để thêm hóa đơn cho hợp đồng này
                                </p>
                            </div>
                        </div>
                    </td>
...
@if($contractInvoices->count() > 0)
    <div class="fi-ta-footer px-3 py-4 sm:px-6">
        <div class="fi-ta-pagination-summary text-sm leading-6 text-gray-600 dark:text-gray-400">
            Tổng số hóa đơn: <span class="font-medium">{{ $contractInvoices->count() }}</span>
        </div>
    </div>
@endif